 ...
 * Supports compact mode for grouped views.
 */
import StatusBadge from './StatusBadge'

function AreaCard({ area, onEdit, onDelete, onViewQR, compact = false }) {
  ...
  // Compact version for grouped views
  if (compact) {
    return (
      <div className="bg-neutral-50 rounded-xl p-4 border border-neutral-100">
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-sm font-semibold text-neutral-900">{area.name}</h4>
        ...
        <div className="flex items-center gap-4 mb-3">
          <div>
            <span className="text-lg font-semibold text-neutral-900">{area.currentCount}</span>
            <span className="text-neutral-400 text-sm"> / {area.capacity}</span>
          </div>
